        ...
        <div className="space-y-12">
          {projects.map((project, index) => (
            ...
            >
              {/* Project Info */}
              <div className="flex-1">
                <h3 className="font-semibold text-[#1a1a1a] hover:text-[#bcff4f] md:text-lg">
              ...
              {/* Technology Tags */}
              <div className="flex shrink-0 flex-wrap gap-2 md:w-48 md:justify-end">
                {project.technologies.map((tech, techIndex) => (
                  <span
                    key={techIndex}
                    className={`rounded-full px-3 py-1 text-xs font-medium text-white ${getTagColor(tech)}`}
                  >
                    {tech}
                  </span>
                ))}
